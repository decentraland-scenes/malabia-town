...
// Random number between 2 numbers
export function randomIntExcluding(number1: number, number2: number, exclude: number) {
    if (number1 == number2) {
        return number1
    }
...
export function blenderTransform(blenderT: any) {
    ...
    let scale = new Vector3(blenderT.scale.x, blenderT.scale.z, blenderT.scale.y)
