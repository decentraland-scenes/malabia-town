...
export class ElectricidadComponent {
    entity_frames: Array<Entity>
    frame: number
    base_transform: Transform
    gltf_frames: Array<GLTFShape>
    looping: boolean
    playing: boolean
    constructor() {
        this.entity_frames = []
...
export class Electricidad extends Entity {
    constructor(electricidad_pasillo_frames: Array<GLTFShape>) {
        super()
        ...
        for (let n=0; n<electricidad_pasillo_frames.length; n++) {
            ...
            elect_component.entity_frames[n].addComponent(electricidad_pasillo_frames[n])
...
export class ElectricidadSystem implements ISystem {
    ...
    update(td: number) {
        ...
        this.timePass = 0
        for (let elect of electGroup.entities) {
            ...
            if (!elect_component.playing) {
                for (let n=0; n<elect_component.entity_frames.length; n++) {
                    const frameTransform = elect_component.entity_frames[n].getComponent(Transform)
